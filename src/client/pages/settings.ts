(() => {
  const token = (): string | null => localStorage.getItem('kingbotToken');
  ...
  const markCurrentPlan = (current: string): void => {
    document.querySelectorAll<HTMLButtonElement>('.plan-btn').forEach((button) => {
  ...
  document.querySelectorAll<HTMLButtonElement>('.plan-btn').forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      try {
        ...
        if (activated) {
          document.querySelectorAll<HTMLButtonElement>('.plan-btn').forEach((btn) => {
            btn.disabled = false;
  ...
  const fillProfile = (user: KingBotUserShape): void => {
    ...
    if (dn) dn.textContent = user.name || 'Trader';
    if (de) de.textContent = user.email || '—';
    if (plan) plan.textContent = user.planLabel || (user.plan || 'free').toUpperCase();
    if (av) {
      if (user.avatar) {
  ...
  const loadMe = async (): Promise<void> => {
    ...
    try {
      ...
      if (!res.ok) return;
      const data = await res.json();
      if (data.user) fillProfile(data.user);
    } catch {
      window.KingBotUI?.toast('Could not load profile', 'error');
    }
  };

  (document.getElementById('profileForm') as HTMLFormElement | null)?.addEventListener('submit', async (e) => {
    e.preventDefault();
    const t = token();
    if (!t) {
      window.location.href = '/login';
    ...
    const name = (document.getElementById('profileNameInput') as HTMLInputElement | null)?.value?.trim();
    const email = (document.getElementById('profileEmailInput') as HTMLInputElement | null)?.value?.trim();
    ...
    try {
      ...
      if (nameEl) nameEl.textContent = data.user.name || 'Trader';
      if (avatarEl) {
        if (data.user.avatar) avatarEl.innerHTML = `<img src="${data.user.avatar}" alt="" />`;
        else avatarEl.textContent = (data.user.name || 'T').trim().charAt(0).toUpperCase();
      }
  ...
  const risk = document.getElementById('riskPercent') as HTMLInputElement | null;
  ...
  const syncPreview = (): void => {
    ...
    map.forEach(([id, pid]) => {
      const a = document.getElementById(id) as HTMLSelectElement | null;
    ...
    if (risk && riskVal) {
      ...
      if (pr) pr.textContent = `${risk.value}%`;
    }
  };
