(() => {
  ...
  if (location.hash) {
    ...
    if (document.querySelector(`[data-section-panel="${id}"]`)) showSection(id);
  }
  ...
  const renderUsers = (list: AdminUser[]): void => {
    const body = document.getElementById('usersBody');
    const subs = document.getElementById('subsBody');
    const label = document.getElementById('userCountLabel');
    if (label) label.textContent = `${list.length} user${list.length === 1 ? '' : 's'}`;
    const row = (u: AdminUser): string => {
  ...
  const load = async (): Promise<void> => {
    try {
      ...
      if (healthRes.ok) {
        ...
        if (dbEl) dbEl.className = h.db === false ? 'status-off' : 'ok';
      } else {
