(() => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const contentHost = shellRoot.querySelector<HTMLElement>('[data-app-content]') || shellRoot;
    ...
    const toggleSidebar = (force?: boolean): void => {
    ...
    fetch('/api/me', { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then(({ user }: { user: KingBotUserShape }) => {
        ...
        document.dispatchEvent(new CustomEvent<KingBotUserShape>('kingbot:user', { detail: user }));
    ...
    fetch('/api/markets')
      .then((res) => res.json())
      .then(({ markets }: { markets: MarketItem[] }) => {
    ...
    if (notifList) {
      notifList.innerHTML = NOTIFICATIONS.map(
        (item) => `<div class="notif-item"><strong>${item.title}</strong>${item.body}</div>`
      ).join('');
    }
    const toggleNotif = (open?: boolean): void => {
      ...
      if (shouldOpen) notifBtn.querySelector('.dot')?.remove();
    };
