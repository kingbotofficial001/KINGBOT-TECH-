(() => {
  ...
  const drawSpark = (canvas: HTMLCanvasElement | null, points: number[], color: string): void => {
    if (!canvas) return;
  ...
  const randomSeries = (base: number, count: number, volatility: number): number[] => {
    const series = [base];
  ...
  const buildCandles = (base: number, count: number, volatility: number): Candle[] => {
  ...
  const drawCandles = (
    ...
    candles: Candle[]
  ): { sellY: number; buyY: number; sellPrice: number; buyPrice: number } | null => {
    ...
    const toY = (price: number): number => padTop + usableH - ((price - min) / range) * usableH;
  ...
  const renderMainChart = (): void => {
    ...
    if (sellTag) {
      ...
      if (span) span.textContent = result.sellPrice.toFixed(2);
    }
    if (buyTag) {
      buyTag.style.top = `${Math.min(Math.max(result.buyY, 10), wrapHeight - 30)}px`;
      const span = buyTag.querySelector('span');
      if (span) span.textContent = result.buyPrice.toFixed(2);
    }
  };

  const renderHeroTicker = (markets: MarketItem[]): void => {
  ...
  const wireTimeframeTabs = (): void => {
    document.querySelectorAll<HTMLButtonElement>('#timeframeTabs button').forEach((tab) => {
  ...
  const loadDashboard = async (): Promise<void> => {
    ...
    const { user } = (await meRes.json()) as { user: KingBotUserShape };
    const analytics: AnalyticsData | null = analyticsRes.ok ? await analyticsRes.json() : null;
    const botsData = botsRes.ok ? ((await botsRes.json()) as { bots: unknown[] }) : { bots: [] };
    const positionsData = positionsRes.ok ? ((await positionsRes.json()) as { positions: Position[] }) : { positions: [] };
    if (marketsRes.ok) {
      const { markets } = (await marketsRes.json()) as { markets: MarketItem[] };
      renderHeroTicker(markets);
    }
    const welcomeName = document.getElementById('welcomeName');
    if (welcomeName) welcomeName.textContent = `Welcome back, ${user.name || 'Trader'}`;
    ...
    if (marketSession) {
      ...
      marketSession.innerHTML = sessions
        .map(
          (session) =>
            `<div class="kv-row"><span class="kv-label">${session.name}</span><span class="stat-pill" style="${session.open ? '' : 'opacity:0.5;'}">${session.hours}</span></div>`
        )
        .join('');
    }
    ...
    drawSpark(document.getElementById('performanceChart') as HTMLCanvasElement | null, randomSeries(1000, 24, 90), '#33d68b');
    renderMainChart();
  };
  ...
  const startLiveEngine = (): void => {
    const topClock = document.getElementById('topClock');
    const sessionEl = document.getElementById('sessionClock');
    const tickTop = (): void => {
      const now = new Date();
      if (topClock) topClock.textContent = now.toLocaleTimeString('en-GB');
      if (sessionEl) {
        const h = now.getUTCHours();
        let session = 'ASIAN SESSION';
        if (h >= 7 && h < 13) session = 'LONDON SESSION';
        else if (h >= 13 && h < 21) session = 'NY SESSION';
    ...
    setInterval(() => {
      ...
      if (confBar) confBar.style.width = `${Math.round(confidence)}%`;
    }, 3200);
