...
(() => {
    ...
    const pendingRef = (() => {
        ...
        if (fromUrl)
            localStorage.setItem('kingbotRef', fromUrl.trim().toUpperCase());
    ...
    const submitGoogleCredential = async (credential, acceptedTerms) => {
        try {
    ...
    const signupSubmit = signupForm?.querySelector('button[type="submit"]') || null;
    const signupTerms = document.getElementById('termsCheckbox');
