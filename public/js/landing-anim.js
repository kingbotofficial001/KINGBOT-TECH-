...
(() => {
    ...
    function reveal() {
        const nodes = document.querySelectorAll('[data-cinematic]');
        if (prefersReduced) {
    ...
    function parallaxBrain() {
        if (prefersReduced)
            return;
        const img = document.querySelector('.hero-brain-img');
        const stage = document.querySelector('.hero-brain-stage');
        if (!img || !stage)
            return;
        let targetX = 0;
        let targetY = 0;
    ...
    function scrollParallax() {
        if (prefersReduced)
            return;
        const bg = document.querySelector('.ambient-brain');
        const hero = document.querySelector('.hero-brain-stage');
        let ticking = false;
        window.addEventListener('scroll', () => {
            ...
            requestAnimationFrame(() => {
                ...
                if (bg)
                    bg.style.transform = `translate(-50%, ${y * 0.08}px)`;
                if (hero) {
    ...
    function particles() {
        ...
        const frame = () => {
            ...
            for (let i = 0; i < nodes.length; i += 1) {
                ...
                n.y += n.vy;
                if (n.x < -20)
                    n.x = window.innerWidth + 20;
                if (n.x > window.innerWidth + 20)
                    n.x = -20;
                if (n.y < -20)
                    n.y = window.innerHeight + 20;
                if (n.y > window.innerHeight + 20)
                    n.y = -20;
    ...
    function initHeroVisible() {
        const hero = document.querySelectorAll('.hero-neural [data-cinematic]');
        hero.forEach((el, i) => {
